...
public class Algoritimo_2 {
    static void main(String[] args) {
        /*2. Soma de matrizes
        Implemente um algoritmo que realize a soma de duas matrizes de mesma ordem.
                O programa dever ́a:
        • verificar se a soma  ́e poss ́ıvel;
        • calcular
        ...
        if (linhas1 == linhas2){    // se a quantidade de linhas das duas matrizes forem iguais passa
            if (colunas1 == colunas2){  // se a quantidade de colunas das duas matrizes forem iguais passa (se a quantidade de linhas e colunas forem iguais portanto tem o mesmo tamanho)
                ...
                for (int i = 0; i<linhas1 ; i++){        // laço pra calcular a matriz 3 que é a soma das duas anteriores
                    for (int j = 0; j <colunas1 ; j++){
                        matrix3[i][j] = matrix1[i][j] + matrix2[i][j];  // calcula a matriz 3 somando os valores das matrizes 1 e 2 da mesma posição
                    }
                }
                System.out.println();
                System.out.println("-----Soma (Matriz 1 + Matriz 2)-----");     // separação para mostrar a soma das matrizes
